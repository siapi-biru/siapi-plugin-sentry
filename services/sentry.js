...
const createSentryService = () => {
  ...
  return {
    ...
    init() {
      ...
      try {
        ...
        if (settings.dsn) {
          Sentry.init({
            dsn: settings.dsn,
            environment: siapi.config.environment,
            ...settings.init,
          });
          ...
          siapi.log.info('siapi-plugin-sentry is disabled because no Sentry DSN was provided');
        }
      } catch (error) {
        siapi.log.warn('Could not set up Sentry, make sure you entered a valid DSN');
      }
    ...
    sendError(error, configureScope) {
      ...
      if (!isReady) {
        siapi.log.warn("Sentry wasn't properly initialized, cannot send event");
        return;
      }

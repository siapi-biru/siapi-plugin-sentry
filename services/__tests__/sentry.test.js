...
describe('siapi-plugin-sentry service', () => {
  beforeEach(() => {
    // Reset Siapi state
    global.siapi = {
      config: {},
  ...
  it('disables Sentry when no DSN is provided', () => {
    sentryService.init();
    expect(siapi.log.info).toHaveBeenCalledWith(expect.stringMatching(/disabled/i));
  ...
  it('disables Sentry when an invalid DSN is provided', () => {
    global.siapi.plugins.sentry.config = {
      dsn: INVALID_DSN,
    };
    sentryService.init();
    expect(siapi.log.warn).toHaveBeenCalledWith(expect.stringMatching(/could not set up sentry/i));
  ...
  it("doesn't send events before init", () => {
    ...
    expect(siapi.log.warn).toHaveBeenCalledWith(expect.stringMatching(/cannot send event/i));
  });
